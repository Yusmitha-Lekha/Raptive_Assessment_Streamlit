...
with tab1:
    ...
    with colR:
        ...
        if dist_name != "Cauchy":
            ...
            osm, osr = stats.probplot(means, dist="norm", sparams=(mu, sigma/np.sqrt(n)))
        else:
            # fall back to empirical normalization just to visualize
            m_emp, s_emp = np.mean(means), np.std(means)
            osm, osr = stats.probplot((means - m_emp)/s_emp, dist="norm")
        axqq.scatter(osm[0], osr, s=10, alpha=0.6)
        minv, maxv = np.min(osm[0]), np.max(osm[0])
